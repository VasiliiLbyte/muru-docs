...
**Статус:** **CUTOVER ВЫПОЛНЕН 2026-07-14.** Прод: `master @ 1392cb7`, `CATALOG_SOURCE=crm`, финальный sync 264 SKU.
...
| 3 | Прод `.env`: `CDEK_SENDER_ADDRESS` — проверить кавычки (TD-003) | ✅ 2026-07-14 |
| 4 | Прод `.env`: нет дубликатов `CATALOG_SOURCE` (иначе выигрывает неожиданная строка) | ✅ 2026-07-14 (окно) |
| 5 | Staging `/var/www/muru-staging` синхронизирован с `master @ 1392cb7` (TD-002) | ✅ 2026-07-14 |
| 6 | Договорённость с клиентом о freeze таблицы: дата/время; после — Sheets только чтение | ☐ (операционно) |
| 7 | Бэкап прод-БД непосредственно перед окном (`pg_dump muru_db`) | ✅ 2026-07-14 (`177K`, `/root/backups/`) |
...
**Итог 2026-07-14:** п.2, 3*, 4, 6, 7 ✅; п.5 ⏸ (баг admin `products/new` route); п.8 — подтвердить TG sync 423.

| # | Проверка | Ожидание | 2026-07-14 |
|---|---|---|---|
| 0 | `pm2 logs muru-backend` — строка `[env] loaded:` | **`/var/www/muru/.env`** | ✅ |
| 1 | `curl -sS http://127.0.0.1:4000/api/health` | 200 | ✅ |
| 2 | Mini App → инвойс `https://t.me/$` | OK | ✅ |
| 3 | Витрина: каталог, карточка, корзина | OK | ✅* (CRM-фото — follow-up storefront) |
| 4 | CRM writable (не read-only) | OK | ✅ |
| 5 | Создать товар + фото → оба канала | OK | ⏸ баг `isNew` на `products/new` |
| 6 | `/img/<fileId>` | 200 | ✅ `crm_7498…` |
| 7 | Архив → исчез из каналов | OK | ✅ |
| 8 | TG sync → 423 | OK | ☐ |
| 9 | `MINIAPP_MAINTENANCE=false` | OK | ✅ |
