...
- **MERGED → `main` @ `459eb90`** (2026-07-31): FF `feat/banner-video-sf` → `origin/main` (M7+M8+M8-9+kitchen+collections+banner video).
- Было staging на feature; **VPS:** переключить `/var/www/muru-storefront` на `main` @ `459eb90` (если ещё на feature-ветке).
- DEP-051/052 merge gate **закрыт**; tip включает video (DEP-054 SF).
...
- **DEPLOYED + smoke OK** (2026-07-31): BE `8e146b9`; SF теперь на **`main` @ `459eb90`** (после FF merge).
- VPS staging: checkout `main` если ещё `feat/banner-video-sf`.

### Баги витрины (диагностика 2026-07-31)
1. **Кухня hub → listing:** **ACCEPT** @ `51e944a` → в `main` `459eb90`.
2. **Коллекции пустые:** **ACCEPT** @ `51e944a` → в `main` `459eb90`.
- **Next:** VPS `git checkout main && pull` + rebuild (если tip ещё feature).
...
Ветка `feat/mobile-adaptation` tip **`516b5b7`**. Эталон: muru.ru mobile UA (поведение/метрики).

| Этап | SHA | Итог |
|---|---|---|
| M0 harness | `b5b2b61` | red baseline: overflow 14, tap ~190, zoom 5 |
| M1 tokens | `57060c3` | zoom 0; tap 161; overflow 12 |
| M2 header | `c050b78` | overflow 0; search overlay; menu account |
| M3 catalog | `2afb752` | grid 2/3/4; filter bottom sheet |
| M4 PDP | `e26ee26` | snap gallery; sticky buy; related carousel |
| M5 commerce | `51a8e5b` | basket/checkout sticky; account nav |
| M6 gate | `516b5b7` | home/footer/content; **full green** |
...
**BE** `master` @ **`69c83aa`**. **SF** `main` @ **`99db9c3`** (W-SEC tip был `4d06417`; +nav accent).
...
### Кандидаты на следующее (после фазы W / параллельный ops)
...
| DEP-051 | **M7 mobile visual polish** M7-1…6 | `muru-storefront` / `main` (`459eb90`) | verified | **merged** ✅ (2026-07-31); VPS confirm checkout main | FF feat/banner-video-sf → main | includes M8+video tip |
...
- **2026-07-28 (сессия 45, ACCEPT M2 → M3):** **ACCEPT `2026-07-28-13-m2`** @ `c050b78`. Verify: single-row header; search Dialog; menu account/counts; overflow 0; tap 156. Выдан **`2026-07-28-14-m3`**.
- **2026-07-28 (сессия 46, catalog parity):** Аудит Sheet ↔ CRM/web.murushop.ru ↔ muru.ru. Скрипт `scripts/catalog-parity-audit.py`; отчёт `CATALOG_PARITY_AUDIT.md`; CSV/JSON в `audits/catalog-parity-2026-07-28/`. Counts: Sheet **325**, CRM **264**, Bitrix PDP **229**. Missing in CRM: **38 REAL** (`MU0296`–`MU0333`) + **23 STUB**; name diffs **3**; price diffs **5**; photo Sheet>CRM **91**; CRM behind Bitrix within cap **83**. **Verdict:** таблицу отрезать нельзя до импорта REAL + photo/name/price backfill. Mobile M0–M6 не затронут.
- **2026-07-28 (сессия 45, ACCEPT M1 → M2):** **ACCEPT `2026-07-28-12-m1`** @ `57060c3`. zoom 5→0, tap→161, overflow 14→12. Выдан **`2026-07-28-13-m2`**.
- **2026-07-28 (сессия 45, ACCEPT M0 → M1):** **ACCEPT `2026-07-28-11-m0`** @ `b5b2b61`. Baseline red. Выдан **`2026-07-28-12-m1`**.
- **2026-07-28 (сессия 45 start):** Открыт эпик **Mobile adaptation M0–M6**. Выдан **`2026-07-28-11-m0`**.
- **2026-07-28 (сессия 44c):** **ACCEPT `2026-07-28-10`** — `/catalog/` root tiles `aspect-square` (+ skeleton). SF `main` uncommitted. Next: commit+push → VPS.
- **2026-07-28 (сессия 44b):** **ACCEPT `2026-07-28-09`** — «Товары» в сайдбар admin. Ветка `feature/admin-sidebar-products`: CatalogRootLayout + ProductsLayout; products вне CatalogLayout tabs; exclusive active. `tsc -b`+`build` OK (оркестратор). Uncommitted. Next: commit → FF master → DEP → deploy.sh.
- **2026-07-28 (сессия 44):** **ACCEPT `2026-07-28-08`** nav brand accent. Diff 4 files OK (`NavItem.accent`, `NavLink`, `CatalogMenuPanel` + `isSaleCategorySlug`); `tsc` clean. Commit+push `99db9c3` → `origin/main`. **DEP-047 pending VPS.** Mobile / drawer left «Новинки» — out of scope.
...
- **2026-07-22 (сессия 41, G2 ACCEPT → W3):** **ACCEPT `2026-07-22-04-w2`**. BFF `[...path]`, cookie `muru_customer_rt`, access in-memory, `proxy.ts` (Next 16), strip refresh, XFF tests. typecheck OK; vitest **14/14**; mock build OK (`ƒ Proxy`, `ƒ /api/account/[...path]`, stubs). Soft: empty cookie name still matches guard (редко). Выдан **`2026-07-22-05-w3`**.
- **2026-07-22 (сессия 41, G1 ACCEPT → W2):** **ACCEPT `2026-07-22-03-w1-fix`**. Fixes 1–4 verified (orphan DELETE, atomic used_at, changePassword revoke, verifySmtpTransport + prod exit). tsc OK; vitest **483/483**. W1 код всё ещё uncommitted на `feature/phase-w-auth`. Выдан **`2026-07-22-04-w2`**.
- **2026-07-22 (сессия 41, W1 review HOLD):** Independent verify `feature/phase-w-auth` (uncommitted). tsc OK; vitest **476/476**; 031 applied local (customers + xor favorites). I2/I4 tests present; `/api/account` mount без maintenance; I1 favorites ON CONFLICT partial OK; snapshots не тронуты. **Блокеры G1:** (1) register orphan при fail sendVerifyEmail → 503 + 409 на повтор; (2) verify/reset one-shot не атомарны; (3) `changePassword` не revoke refresh (в отличие от reset); (4) нет SMTP `verify()` при старте (было в W1 ТЗ). Выдан **`2026-07-22-03-w1-fix`**.
- **2026-07-22 (сессия 41, G0 ACCEPT → W1):** **ACCEPT `2026-07-22-01-w0`**. Verify оркестратора: `POST /api/auth/telegram` only; mounts+maintenance OK; favorites JWT `telegramId` + NOT NULL; SF no BFF + `muru-favorites`; `sendEmail` только из мёртвого `notifyByEmail`; silent SMTP confirmed `email.service.ts:28-34`; SQL 0.6 корректны (`status='succeeded'`, `checkout_snapshot->>'email'`). SHA `37d8065`. Выдан **`2026-07-22-02-w1`**.
- **2026-07-22 (сессия 41, W0 issued):** Оркестратор активировал фазу W. Defaults: order-status email — не в W1; «logout everywhere» UI — позже (W1 = инвалидация текущего refresh). Выдан **`2026-07-22-01-w0`** (read-only, BE + RO SF п. 0.5). Ждём отчёт → G0.
- **2026-07-22 (сессия 41 start):** Открыта **фаза W — личный кабинет покупателя**. Решения зафиксированы в новом `PHASE_W.md` (v1 = email+пароль + SmartCaptcha + double opt-in; SMS-OTP отложен в W5, альфа-имя подано на модерацию 2026-07-22). SPEC §14 «SMS vs email» закрыт. Найдены два блокера в схеме: у `orders` нет ни email, ни телефона (только `telegram_user_id` NULL + `channel`) → историю заказов в ЛК показать нечем; `favorites.telegram_user_id NOT NULL` → избранное недоступно веб-пользователю. Миграция 031 не аддитивна ⇒ **staging-first обязателен**. Мина: `email.service.ts` молча не отправляет письма при неконфигурированном SMTP.
